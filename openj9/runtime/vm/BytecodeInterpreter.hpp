...
unsigned int s_add, s_sub, s_mul, s_div;
unsigned int s_func_calls;
...
noUpdate:
		...
		getMethodName(PORTLIB, _sendMethod, (U_8*)-1, currentMethodName);
		printf("METHOD -> %s\n", currentMethodName);
		printf("/*---------------STATS --------------*/\n add %d, sub %d, mul %d, div %d, func %d\n total arithmatic %d\n", s_add, s_sub, s_mul, s_div,s_func_calls, s_add + s_sub + s_mul + s_div);
		return _nextAction;
	}
